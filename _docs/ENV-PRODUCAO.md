...
> Referência das chaves esperadas no painel Hostinger (hPanel → demandoo.net → Variáveis de ambiente).
...
| `NEXTAUTH_URL` | fixo | `https://demandoo.net`. A chave `NEXTAUTH_URL` **continua válida no v5** (lida como alias de `AUTH_URL`, ver `node_modules/next-auth/lib/env.js`). Só o `NEXTAUTH_SECRET` é que foi aposentado |
...
| Chave | Origem do valor | Observação |
|---|---|---|
| `NEXT_PUBLIC_APP_URL` | fixo | `https://demandoo.net`. **Injetado no build** — ver seção 3 |
...
local. Em produção são `https://demandoo.net`. Nunca copiar esses dois do `.env.local`.
