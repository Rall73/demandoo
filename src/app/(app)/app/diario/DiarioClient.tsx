...
export default function DiarioClient({
  ...
  const prevData = addDias(dataISO, -1)
  ...
  // Scroll para o fim da timeline ao carregar
  ...
  async function excluirEntrada(id: number) {
    ...
    try {
      ...
      // entrada já removida da UI — ignora erro silencioso
    } finally {
      setLoadingDel(null)
    }
  }
  ...
  async function criarDemanda(entrada: EntradaDiario) {
    ...
    try {
      const res  = await fetch("/api/demandas", {
        ...
        body:    JSON.stringify({
          manual:    true,
          titulo:    entrada.conteudo.slice(0, 80),
          descricao: entrada.conteudo,
        }),
      })
      const data = await res.json()
    ...
    <div className="flex flex-col h-full p-4 md:p-8 max-w-7xl">
      ...
      <div className="flex flex-col md:flex-row gap-5 flex-1 min-h-0">
        ...
        <div className="md:w-2/5 flex flex-col gap-4 overflow-y-auto">
          ...
          {/* Ações */}
          {(acoes.length > 0 || acoesIniciais.length > 0) && (
            <section>
              <h2 className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2.5 flex items-center gap-1.5">
              ...
              <div className="flex flex-col gap-2">
                {acoes.map((a) => {
                  ...
                  return (
                    <div
                      key={a.id}
                      className={`flex items-start gap-2.5 bg-white border rounded-xl p-3 transition-all ${
                        a.feita ? "border-slate-100 opacity-50" : "border-slate-200"
                      }`}
                    >
                      <button
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-1 mt-0.5">
                          <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${cfg.cor}`}>
                    ...
                    {!isPending && (
                      <div className="shrink-0 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                        <button
                          ...
                          className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs font-medium text-violet-600 hover:bg-violet-100 transition-colors disabled:opacity-50"
                        >
                          {loadingDem === e.id
          ...
          <div className="border-t border-slate-100 p-3">
            ...
            <div className="flex gap-2 items-end">
              <textarea
                ...
                onKeyDown={(e) => {
                  if (e.key === "Enter" && (e.ctrlKey || e.metaKey)) adicionarEntrada()
                }}
            ...
            <p className="text-[10px] text-slate-400 mt-1.5 ml-1">
              Ao clicar em "Demanda" num registro, uma demanda é criada automaticamente com o conteúdo.
            </p>
          </div>
        </div>
      </div>
